...
var checkIncorrectAnswer = function (result, parsonInterface) {
    ...
    //else result includes the substring of '100%' in results/answer is correct
        ...
        //if the quesiton is answered correctly the first time without clicking the hing button, you cannot continue to spam click 
        //the buttons that writes to the firebase
        $("#execute").addClass("invisible");
    ...
    if (attempts == 5 && hintCalled == false && parsonInterface.includes(falseBool)) {
        write_Hint_And_Execute_Timestamps_To_FireBase(hintCalled, attempts, newQuestionO, questionNumber);
        console.log("You have answered a question incorrectly " + attempts + " times.");
        //attempts += 1;
...
var checkAsserts = function (result_table, test_list) {
    ...
    for (var i = 0; i < 1; i++) {
        ...
        for (var j = 0; j < 2; j++) {
            if (j == 0) {
                //read and removes the first line from the text file(questionOutput was already removed) and assigns
                //it to parsonInterfaceInput
                ...
                //if parsonInterface string includes "false", then the hint button/Parson problem is not available for the question 
            ...
            //if j == 1, reads and removes the second line from the text file and assigns it to testCaseInterfaceInput
            //unfortunately we didnt have enough time to disable the test cases correctly without error. We beleive that with enough
            //we would diable addPassMessage and addFailMessage
            else if (j == 1) {
                var testCaseInterfaceInput = tests.shift();
                //makes testCaseInterfaceInput to lowercase and assign it to testCaseInterface
                testCaseInterface = testCaseInterfaceInput.toLowerCase();
    ...
    for (var i = 1; i < 3; i++) {
        //if i is 1, reads and removes the first line and assigns it to code_Word
        if (i == 1) {
            this.code_Word = tests.shift();
        }
        //if i is 2, reads and removes the 2nd line and assigns it to p(arson)p(roblem)Code_Word
        else {
    ...
    //if the answer is correct and hint button was not clicked, decrypt and output the free form text code word
    if (this.failed == 0 && hintCalled == false) {
        result += "\nCodeword is " + Decryption(code_Word);
        console.log("this is the code word " + code_Word);

    }
    //if answer is correct and hint button was clicked, decrypt and output the Parsons Problem code word
    else if (this.failed == 0 && hintCalled == true) {
...
var setGlobalVariable = function (test_list) {
    var test = test_list;
    //finds where 'Parsons' is listed in the text file, seperates it into an array and assigns it to parsonTextSeperator
    parsonTextSeperator = test.split('Parsons');
    //assigns everything after 'Parsons' to parsonProblem
    parsonProblem = parsonTextSeperator[1];
    //removes undesirable characters from parsonProblem
    parsonProblem = parsonProblem.replace(/^\s+|\s+$/g, '');
